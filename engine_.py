...
class Pokemon:
    ...
    def get_xp_growth_rate(self, n=None):
        """
        ...
        n = self.level + 1 if not n else n
...
class Battle:
    def __init__(self, ally, enemy):
        ...
        self.enemy = enemy
        self.active = True
        self.ally_turn = self.get_turn()  # Determina de quien es el turno. En un inicio, ataca el pokemon mas rapido
    ...
    def get_bonus_and_effectivity(self, move):
        ...
        e = self.effect_chart[move.type_id][defender.id_type[0]]
        return b, e
    ...
    def end_combat(self):
        ...
        :return:
        """
        if self.ally_turn:  # Ganó enemy
            # self.enemy.current_xp += self.get_experience()
            # if self.enemy.current_xp >= self.enemy.xp_next_level:
            #     self.enemy.level += 1
            pass
        else:  # Ganó ally
            self.ally.current_xp += self.get_experience()
            if self.ally.current_xp >= self.ally.xp_next_level:
                self.ally.level += 1
